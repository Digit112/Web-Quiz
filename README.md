...
- `children-give-incorrect-answers` (optional; default `true`): If true, the answers to descendants of this group can appear as incorrect responses to other descendants of this group, when those other descendants are presented as multiple-choice.
...
- `correct-answer-source` (optional; default `"primary"`): Where, in multiple choice presentation, the displayed correct option is chosen from. Can be either `"primary"`, in which case the primary answer is always chosen, or `"random"`, in which case a random answer is chosen.
- `incorrect-answers` (optional): A list of incorrect answers which will be displayed to the user as options in multiple-choice presentation. Replaces incorrect answers generated by ancestors.
